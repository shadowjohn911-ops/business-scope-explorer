...
const generateRate = () => Math.floor(Math.random() * 200) - 100;
...
const generateDetails = () =>
  Array.from({ length: 5 }, (_, i) => ({
    ...
    entryDate: `2026-0${Math.floor(Math.random() * 3) + 1}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, "0")}`,
    firstSwipeDate: `2026-0${Math.floor(Math.random() * 3) + 1}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, "0")}`,
          ...
          <div className="flex items-center justify-between">
            ...
            <Popover>
              ...
              <PopoverContent className="w-72 text-[11px] leading-relaxed text-foreground" side="left" align="start">
                表格中的数值表示在指定回溯周期内（如近7日、近30日）发生过该行为的商户数量；百分比表示当前周期相较于上一周期的环比变化率（正数表示增长，负数表示下降）。数值小于100的单元格，可点击查看明细。明细字段为：商户号、商户名称、入网日期、首刷日期、末刷日期。排序字段：入网日期、首刷日期、末刷日期。
              </PopoverContent>
            </Popover>
          </div>
        </CardHeader>
                ...
                <tr className="border-b border-border">
                  ...
                  <th className="relative w-16 h-10 p-0 overflow-hidden">
                    <svg className="absolute inset-0 w-full h-full" viewBox="0 0 64 40" preserveAspectRatio="none">
                      <line x1="0" y1="40" x2="64" y2="0" stroke="hsl(var(--border))" strokeWidth="1" />
                    </svg>
                  ...
                  <tr key={b} className="border-b border-border last:border-0">
                    ...
                    {periods.map((p) => {
                      ...
                      return (
                        <td
                          key={p}
                          className={`text-center py-2 px-1 ${clickable ? "cursor-pointer hover:bg-muted/50 rounded" : ""}`}
                          onClick={() => handleCellClick(b, p, value)}
                        >
                          <div className={`font-semibold ${clickable ? "text-primary underline underline-offset-2" : "text-foreground"}`}>
                            {value}
                          </div>
                          <div

...
// ⬆200%以上 must match "交易额波动超200%的服务商" counts: 昨日2, 近7日12, 近30日22, 近90日35
const mockData: Record<string, Record<string, { value: number; rate: number }>> = {
  ...
  "-50%~50%":    { "昨日": { value: 20, rate: -3 }, "近7日": { value: 22, rate: -5 },  "近30日": { value: 28, rate: 3 },   "近90日": { value: 32, rate: 5 } },
  "⬇50%~75%":   { "昨日": { value: 3, rate: -20 }, "近7日": { value: 4, rate: -15 },  "近30日": { value: 6, rate: -18 },  "近90日": { value: 10, rate: -10 } },
  "⬇75%~100%":  { "昨日": { value: 2, rate: -30 }, "近7日": { value: 3, rate: -25 },  "近30日": { value: 4, rate: -28 },  "近90日": { value: 6, rate: -20 } },
  "⬇100%":      { "昨日": { value: 4, rate: 10 },  "近7日": { value: 5, rate: 8 },    "近30日": { value: 8, rate: -5 },   "近90日": { value: 12, rate: -3 } },
};
...
const generateDetails = (band: string, entityLabel: string, count: number) => {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return { providerId: `S${String(1000 + i).padStart(6, "0")}`, providerName: `${entityLabel}${String.fromCharCode(30002 + (i % 20))}${i >= 20 ? i : ""}`, currentAmount, previousAmount, volatility };
  });
};
...
const ProviderTransactionVolatilityTable = ({ entityLabel = "服务商" }: Props) => {
      ...
      <Card className="border-border">
        <CardHeader className="px-3 py-2.5 pb-1">
          <div className="flex items-center justify-between">
            <CardTitle className="text-xs font-semibold text-foreground">{entityLabel}交易洞察</CardTitle>
            <div className="flex items-center gap-1.5">
              ...
              <Popover>
                ...
                <PopoverContent className="w-80 text-[11px] leading-relaxed text-foreground" side="left" align="start">
                  表格中的数值表示在指定回溯周期内（如近7日），交易额较上一周期增长率落在该区间的{entityLabel}数量；百分比表示该区间{entityLabel}数量相较于上一周期的环比变化率（正数表示增加，负数表示减少）。
                  <br /><br /><span className="text-muted-foreground">示例：若"⬆200%以上"对应"近7日"的数值为5，比例为+25%，则表示当前周期（如3/10-3/16）内有5个{entityLabel}的交易额较上一周期（如3/3-3/9）增长超过2倍，且这类{entityLabel}的数量较上一周期增加了25%。</span>
                </PopoverContent>
              </Popover>
            </div>
          </div>

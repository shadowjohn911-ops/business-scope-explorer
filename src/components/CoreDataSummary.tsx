...
function generateOrganizationSummary(period: PeriodType, role: RoleType, ownerLevel1: string): SummaryItem[] {
  const result: SummaryItem[] = [];
  const entityLabel = role === "provider" ? "盟友" : "服务商";
  const showProvider = ownerLevel1 !== "自有";
  ...
  // 3. 交易服务商/盟友 结构 - 仅近30日显示
  if (showProvider && period === "近30日") {
...
const CoreDataSummary = ({ module, period, onPeriodChange, role = "branch", ownerLevel1 = "全部" }: Props) => {
  const items = module === "transaction"
    ? generateTransactionSummary(period, role)
    : module === "organization"
    ? generateOrganizationSummary(period, role, ownerLevel1)
